...
export default function ConnectedUsers({ user, getCurrentUser, currentUser }) {
  ...
  return isConnected ? (
    ...
    <div className="flex flex-col items-center justify-center max-w-lg p-6 bg-white border-2 border-gray-600 shadow-lg rounded-xl xl:rounded-full">
      <img src={user.imageLink} className="block w-20 m-2 rounded-full" />
      <p className="p-2 font-semibold">{user.name}</p>
      <p className="p-2 font-normal ">{user.headline}</p>
